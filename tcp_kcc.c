 ...
 *           The complementary event gives a lower bound on detection power:
 *           P(FA) ≥ 1 − e^{-τ²/(2σ²_noise)}.  At τ = σ: P(FA) ≥ 0.393.
 *           At τ = 2σ: P(FA) ≥ 0.865.  For general zero-mean noise
 *           with bounded density f(0) > 0, P(|T_noise| ≥ τ) ≥ τ·f(0)/2
 *           for sufficiently small τ, ensuring P(FA) > 0 for all finite τ.
 ...
 *     KCC_Rebuttal.md section 6.1.1 for the full treatment.
 *
 ...
 *     has singular FIM (Proof E, at ~L1616), the identifiability question is
 ...
 *       cold-start ramp
 *     - Non-atomic RMW is deliberate: analytically bounded (see KCC_Rebuttal.md §4.6), no locking overhead justified
 *
 ...
 *     (Adapted from KCC_Rebuttal.md B36, §5.10)
 *
 ...
 *         kappa_O   = K_ss*(2-K_ss) = 0.39*1.61 = 0.63 (observer, K_ss=0.39)
 ...
 *       kappa_O = K_ss * (2 - K_ss)
 *             (observer: at nominal K_ss=0.39, kappa_O = 0.39*1.61 = 0.63;
 *              at adaptive K_ss=0.88, kappa_O = 0.88*1.12 = 0.99)
 ...
 * μ = 1 (line 4124) is chosen so both terms have compatible energy units
 ...
 *     At η_max = 5ms (bounded T_noise), the estimation error is
 *     bounded by ≈ 1.05ms — well below typical RTT variability.
 *
 ...
 *     Proof G.1 (lines 12546-12707), where the Lur'e sector-bounded
 ...
 *     classical Lur'e theory.  See §6.1 of KCC_Rebuttal.md for
 ...
 *   Line numbers are approximate and should be verified after edits.
 *
 ...
 *   | Proof B (T_noise Existence) | §Proof B, ~L854 | §Proof B | T_noise physically distinct + Chebyshev |
 *   | Proof C (Directional Update) | §Proof C, ~L879 | §Proof C | Censored gate separates T_prop from T_queue |
 *   | Lemma C.1 (Running-Minimum MLE) | §Lemma C.1, ~L913 | §Lemma C.1 | Running min = MLE under one-sided noise eps≥0 |
 *   | Lemma C.2 (Censored Kalman MMSE) | §Lemma C.2, ~L919 | §Lemma C.2 | CKF = MMSE under Gaussian with one-sided constraint |
 *   | Proof C.1 (Censored Kalman) | §Proof C.1, ~L959 | §Proof C.1 | Tobit-type formulation + a.s. convergence |
 *   | Proof C.2 (Switching KF + NP) | §Proof C.2, ~L1059 | §Proof C.2 | Two-mode SPRT drift detection |
 *   | Proof C.3 (Truncated KF Optimality) | §Proof C.3, ~L1170 | §Proof C.3 | min(0,ν) MMSE-optimal under (A1)-(A3) |
 *   | Proof C.4 (Std KF + Prior = Trunc) | §Proof C.4, ~L1332 | §Proof C.4 | Constrained Kalman w/ ΔT_prop≤0 → truncated KF |
 *   | Proof D (T_noise Isolation) | §Proof D, ~L1586 | §Proof D | Noise enters only attenuated path |
 *   | Proof E (FIM 4-comp Impossible) | §Proof E, ~L1617 | §Proof E | det(I)=0; CRB infinite |
 *   | Proof E1 (Bayesian Cannot Salvage) | §Proof E1, ~L1667 | §Proof E1 | Lambda_post singular on T_prop vs T_queue |
 *   | Proof F (3-comp Identifiable) | §Proof F, ~L1705 | §Proof F | Behavioral priors -> full rank FIM |
 *   | Proof L (Optimality for CC) | §Proof L, ~L583 | §Proof L | Minimal complete signal model; 2-comp fails; 3 is unique |
 *   | Proof M (BBR Degeneracy) | §Proof M, ~L671 | §Proof M | BBR's 2-comp = degenerate KCC 3-comp; Blackwell dominance |
 *   | Proof K (Clean-Sample Starvation) | §Proof K, ~L766 | §Proof K | Fundamental lower bound on T_prop error |
 *   | Corollary K.1 (Starvation Condition) | §Cor K.1, ~L780 | §Cor K.1 | If T_queue>ε for all samples, BDP inflated |
 *   | Corollary K.2 (Graceful Degradation) | §Cor K.2, ~L805 | §Cor K.2 | KCC 3-mechanism composite bound |
 *   | AIC/BIC (Model Selection Vacuous) | §AIC/BIC, ~L1825 | §AIC/BIC | 4-comp likelihood degenerate; selection undefined |
 *   | Theorem 1 (Lyapunov GUAS) | §Thm 1, ~L2688 | §Thm 1 | V(q,d) cycle-average decreasing; GUAS |
 *   | Theorem 2 (Contraction) | §Thm 2, ~L2771 | §Thm 2 | Exponential error decay |
 *   | Theorem 3 (Small-Gain) | §Thm 3, ~L2846 | §Thm 3 | gamma_loop <= K_ss < 1 |
 *   | Theorem 4 (BIBO) | §Thm 4, ~L2867 | §Thm 4 | Bounded input -> bounded output |
 *   | Theorem 5 (ISS Cascade) | §Thm 5, ~L3151 | §Thm 5 | Full closed-loop GAS |
 *   | Corollary (N-flow Fairness) | §Cor, ~L2890 | §Cor | All flows -> C/N |
 *   | Proof I (RTT Asymmetry) | §Proof I, ~L1528 | §Proof I | Bounded-error analysis under asymmetry |
 *   | Proof G.1 (ACK-FSM Observer) | §G.1, ~L12546 | §G.1 | Discrete-time Lur'e + Tsypkin Criterion |
 *   | Proof J (Competition with CCAs) | §Proof J, ~L3098 | §Proof J | BBR/CUBIC/Reno fairness analysis |
 *   | B1-B16 (Boundary Conditions) | §B1-B16, ~L4663 | §B1-B16 | Exhaustive edge-case proofs |
 *   | B17-B28 (Deployment Boundaries) | §B17-B28, ~L4871 | §B17-B28 | Loss, AQM, policer, bufferbloat |
 *   | B29-B35 (Adversarial Cases) | §B29-B35, ~L4984 | §B29-B35 | Reordering, ACK comp, TSO, PIE, CAKE, ECN, PMTU |
 *   | B36-B43 (Competition/Network) | §B36-B43, ~L5061 | §B36-B43 | BBR, ICMP, NAT, cellular, DOCSIS, VPN, LRO, asymmetry |
 *   | B44-B51 (Host Stack + Physical Limit) | §B44-B51, ~L5162 | §B44-B51 | TS wrap, SACK renege, zwp, keepalive, TLP, RACK, PRR, clean-sample starvation |
 *   | Prop 1 (Positive Innovation Bias) | §Prop 1, ~L4365 | §Prop 1 | E[v_k]=mu_q>=0 violates MMSE |
 *   | Prop 2 (Conditional Optimality) | §Prop 2, ~L4379 | §Prop 2 | E[nu_k|nu_k<0]~0 restores MMSE |
 *   | Prop 3 (Drift Correction = SGD) | §Prop 3, ~L4402 | §Prop 3 | SGD with L(x)=1/2*(z-x)^2 |
 *   | Prop 4 (Conservative BDP Bound) | §Prop 4, ~L4461 | §Prop 4 | BDP_KCC <= BDP_true |
 *   | p_ss Model-Mismatch Detector | §p_ss, ~L4422 | §p_ss | p_est > thresh triggers recal |
 *   | Dual-Estimate Maximin | §Dual, ~L4447 | §Dual | model_rtt = min(x_est, min_rtt) |
 *   | Three Lines of Defense Table | §3Lines, ~L4481 | §3Lines | 2 rank-deficiency perspectives + 1 independent behavioral argument |
 *   | Boundary Expansion Analysis | §BEA, ~L4495 | §BEA | T_trans, observability, Bayesian |
 *   | Proof N (5-Scheme Rebuttal) | §Proof N, ~L2054 | §Proof N | All 5 alternatives = special case or dominated |
 *   | Proof O (SIGCOMM'18 Boundary) | §Proof O, ~L2347 | §Proof O | Δ_lo tightened by directional update |
 *   | Mathematical Guarantees Table | §MGT, ~L4548 | §MGT | 28-row comprehensive proof status |
 *
 ...
 *     gain_den < 1 → floor to 1; (d) kcc_kalman_scale > 0 (clamped
 *     to [64, 1048576]); (e) kcc_*_den >= 1 (all denominator params
 ...
 *     (a) sample_cnt: u32, saturated at U32_MAX (line 11914: if
 *     sample_cnt < U32_MAX).  (b) pos_skip_cnt: u8, saturated at
 *     KCC_POS_SKIP_SATURATION (line 11756).  (c) consec_reject_cnt:
 ...
 *     corruption.  (d) BW → ∞ (infinite theoretical): bw_raw capped
     *     at U64_MAX / USEC_PER_SEC before multiplication (line 13753:
 ...
 *   (Adapted from KCC_Rebuttal.md §5.9)
 *
 ...
 *   (Adapted from KCC_Rebuttal.md §5.11)
 *
 ...
 *     structurally overestimated.  See Proof K (~L766) for full analysis.
...
#define BBR_UNIT  (1 << BBR_SCALE) /* [K][T_queue] 256=1.0x gain ref; 2.0x=512; Cardwell et al. 2016 */
     ...
     */
#define KCC_GAIN_SLOTS 256     /* [T_queue] PROBE_BW gain table entries (0..255); 1 slot = 1 RTT; extends kernel BBR's 8-entry cycle */
#define KCC_DECAY_MASK_WORDS 8 /* [T_queue] 256-bit gain-decay mask as 8x32-bit words (disabled by default) */
      ...
      *     Capping at 3e9 provides ~1.2% headroom below the theoretical
      *     overflow boundary.  At kalman_scale = 1024, this corresponds to
      ...
      */
#define KCC_KALMAN_INNOV_SQ_CAP 3000000000ULL /* [K] overflow guard: sqrt(S64_MAX)~3.037e9, capped to 3e9 for innov^2 safety */
...
#define KCC_PROBE_RTT_JITTER_SHIFT      6    /* [T_prop] per-flow jitter shift (/64); jitter range ~0..2 min_rtt */
...
#define KCC_DECAY_BIT_MASK        31  /* [T_queue] gain-decay mask bit position mask: 32-1=31 */
    ...
    */
     ...
     *     (max 3 rounds) and prev_ca_state (max 7 states, using 3 bits).
...
#define KCC_BITFIELD_2BIT_MAX      3    /* [T_queue] max value for 2-bit bitfield (2^2-1=3); full_bw_cnt, prev_ca_state */
#define KCC_GAIN_MAX              1023 /* [T_queue] max 10-bit bitfield value (2^10-1=1023); pacing_gain/cwnd_gain cover 0~4.0x */
#define KCC_LT_RTT_CNT_MAX        4095 /* [K] max 12-bit bitfield value (2^12-1=4095); lt_rtt_cnt ~40s at 10ms RTT */
     /*
      ...
      *
      * Closed-form derivation examples (from KCC_Rebuttal.md §4.8.2):
      ...
      *     factor KCC_KALMAN_SCALE = 1024.  Jitter-EWMA adaptation is a
      ...
      *   - PROBE_BW cycle length: 8 phases, 32 RTTs.  Dwell-time stability
      *     (Liberzon, 2003, Thm 3.1): T_dwell >= tau_min = ln(rho)/ln(1 -
      *     K_ss * p_clean).  With rho=0.01, K_ss=0.39, p_clean=0.3: tau_min
      *     approx 40 RTTs.  32-RTT cycle provides ~5x margin.
      ...
      *
      * Parameter count comparison (from KCC_Rebuttal.md §4.8.4):
...
#define KCC_TSO_HIGH_JITTER_THRESH_US  4000  /* [T_noise] jitter_ewma>4ms: double TSO divisor to maintain pacing */
        ...
        */
#define KCC_AGG_CONFIDENCE_MAX         1024  /* [T_noise] ACK-agg confidence score (0..1024); 10-bit for 4-factor fusion */
...
#define KCC_ALONE_AGG_LEVEL_PERMISSIVE 2    /* [T_noise] alone-on-path strictness: up to CONFIRMED compensation */
  ...
  */
#define KCC_MINRTT_FAST_FALL_DIV_MAX       256   /* [T_prop] minrtt_fast_fall_div upper clamp (2^8, prevents div-by-zero) */
#define KCC_PROBE_BAND_MULT_MAX            32    /* [K] kalman_probe_band_mult upper clamp (2^5, prevents u32 overflow) */
...
#define KCC_MIN_TSO_RATE_DIV_MAX           256   /* [T_queue] min_tso_rate_div upper clamp (2^8, prevents div-by-zero) */
#define KCC_PROBE_RTT_LONG_INTERVAL_DIV_MAX 1000 /* [T_prop] probe_rtt_long_interval_div upper clamp (prevents div-by-zero) */
#define KCC_AGG_CONFIDENCE_ABS_MAX         65536 /* [T_noise] agg_confidence_max abs upper clamp (2^16, prevents div-by-zero) */
...
#define KCC_KF_STARTUP_PROTECT_RTTS 8   /* [K] STARTUP protection rounds: prevent global KF from premature full_bw exit */
#define KCC_KF_OVERFLOW_GUARD   (1ULL << 31) /* [K] global KF: max unscaled P+R before rescaling; keeps (P+R) in 31 bits */
#define KCC_KF_INNOV_SHIFT      10           /* [K] global KF: innov>>2?shift before ratio; prevents 64-bit overflow */
...
#define KCC_DRIFT_TIER2_MULT         8      /* [T_noise] Tier-2 skip multiplier: drift_thresh * 8 = 16 * 8 = 128; derivation: 128 = 2^7 gives Neyman-Pearson Type I error P(false trigger | H_0) = (1/2)^128 = 2.94 x 10^-39 (Proof C.2); the multiplier 8 = 2^3 provides exactly 8x more persistence evidence than Tier 1, ensuring forced correction fires only under statistical certainty of genuine baseline change */
...
#define KCC_TSO_DIV_CEIL             32     /* [T_noise] max TSO divisor for noisy paths */
...
#define KCC_CWND_ABSOLUTE_MIN        1      /* [T_queue] abs min cwnd on loss reduction (1 seg); applied before per-mode floors */
       ...
       *               bbr_probe_rtt_min_us = 10 * USEC_PER_SEC = 10,000,000 us = 10 seconds (from tcp_bbr.c), not 5000ms) expires without
 ...
 * BOUNDS: num [0, 255], den [1, 255]; net multiplier should be >= 1.0x BDP.
 */
 ...
 * BOUNDS: num [0, 255], den [1, 255]; num=0 disables compensation.
 */
 ...
 * (K_ss = 256/(256+400) ≈ 0.39).  The nominal R=400 is an initial
 ...
 * with div = 1000, yields 100x scaling. Default 1000.
 */
static int kcc_kalman_q_rtt_div = 1000;             /* [K] Q adaptation RTT divisor: Q_adapted = Q * max(q_min_factor, min_rtt_us / div); converts min_rtt_us from us to ms for adaptive Q scaling; e.g., 100ms path yields 100x scaling with div=1000; KCC-only; range [1, 1000000], BBR default: N/A */
...
static int kcc_drain_gain_num = 347;                  /* [T_queue] DRAIN pacing gain numerator; 347/1000 = 0.347x float = 88 BBR_UNIT (88/256 = 0.344x); 1/high_gain (1000/2885); kernel BBR: bbr_drain_gain = BBR_UNIT * 1000 / 2885 (tcp_bbr1.c:167); range [0, 100000], BBR default: 347 */
...
static int kcc_kalman_q_scale_cap = 50;               /* [K] cap on Q adaptation factor (min_rtt_us/1000); unified derivation with kcc_kalman_q_max: Q_adapted = Q_base * min(max(q_factor, rtt_us/1000), q_scale_cap).  At cap=50: max Q = 100*50 = 5000.  At this Q and R=400: p_ss = 5372, K_ss = 0.93.  At matched Q=50000, R=32000: p_ss=72170, K_ss=0.69.  The cap prevents Q from diverging on WAN paths by limiting adaptation to 50x base.  Integer bound: 50*100 = 5000 < 100000 (kcc_kalman_q_max parameter clamp), giving 20x headroom before sysctl clamp; KCC-only; range [1, 10000], BBR default: N/A */
 ...
 * BOUNDS: [1, 10000000] = [1 µs, 10 s]; clamped at init (tcp_kcc.c L8338).
...
/*
 * [T_noise] IC note: Maximum LT sampling interval multiplier. The effective max
 * interval = kcc_lt_intvl_min_rtts * kcc_lt_intvl_max_mult = 16 RTTs.
 ...
 * DERIVATION: power-of-two for efficient bit-shift; 10 bits = ~0.1% fractional precision; scale² > max(Q,R,P) = 1,048,576 > 1,000,000 prevents overflow in innov²/scale² division (README.md L2228-2229).
 * BOUNDS: [64, 1048576] = [2^6, 2^20]; clamped and rounded to power-of-two at init (tcp_kcc.c L8381-L8382).
 */
...
static void kcc_init_module_params(void)                          /* clamp all params + compute derived values */
{
    ...
    kcc_qdelay_floor_us = clamp(kcc_qdelay_floor_us, 1, 100000);  /* [T_queue] queuing delay floor */
     ...
     * Both are capped <= 31 so Tier-2 threshold drift_thresh*8 <= 248 fits in u8 (max 254). */
    ...
    kcc_full_bw_cnt_val = (u32)kcc_full_bw_cnt;                                  /* full-BW round count (already clamped to [1,3] at line 8320, fits 2-bit field) */
    ...
    /* Derived Kalman ratios (num/den -> single value) (Kalman 1960) */
    {
        ...
        kcc_kalman_outlier_jitter_mult_val = num / den; /* (num/den) */
    }
    {
        ...
        kcc_kalman_q_min_factor_val = num / den;        /* (num/den) */
    }
    {
    ...
    struct kcc_ext* ext)                                                 /* [T_queue] ext for EWMA storage */
{
    ...
    if (rs->delivered == 0) {
        return;                                                          /* [T_queue] no data in sample */
    }
...
static void kcc_lt_bw_sampling(struct sock* sk, const struct rate_sample* rs)    /* [T_noise] LT bandwidth sampling state machine */
{
     ...
     */
    t_us = div_u64(tp->delivered_mstamp, USEC_PER_MSEC) - kcc->lt_last_stamp;   /* interval duration in ms */
    if ((s64)t_us < 1) {
        return;
    }
    ...
    /* [K] Core Kalman update (Kalman 1960) ---- */
    {
         ...
         */
        {
            ...
            if (innovation < 0 || qboost_fired) {
                ...
                if (!saturation_fired && ext->pos_skip_cnt >= kcc_kalman_drift_thresh_val &&
                ...
                else if (ext->pos_skip_cnt >= kcc_kalman_drift_thresh_val * KCC_DRIFT_TIER2_MULT) {
 ...
 *   (ISS Cascade with Switched-Regime Controller, at ~L3151).
 *
...
static u64 kcc_kf_update(u64 z, u32 r_pct, bool check)                /* feed BW sample into global Kalman filter */
{
    ...
    u64 Pcopy, Rcopy, denom;                                           /* local copies for rescaling; common denominator for Kalman update */
    ...
    x = (x * Rcopy + z * Pcopy) / denom;                               /* innovation-weighted state update: blend prior and measurement */
...
KCC_KFUNC void kcc_init(struct sock* sk)                               /* per-connection init callback [T_prop][T_noise][K] */
{
    ...
    ext = kzalloc(sizeof(*ext), GFP_NOWAIT);                             /* allocate extended state block (never sleeps; safe for process and softirq) */
    if (likely(ext)) {
